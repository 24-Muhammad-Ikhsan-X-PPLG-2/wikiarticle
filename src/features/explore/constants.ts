export const CATEGORIES = [
  "Technology",
  "Science",
  "History",
  "Programming",
  "Religion",
  "Medicine",
  "Art",
  "Sports",
];

export const ARTICLES = [
  {
    title: "The History of Artificial Intelligence",
    category: "Technology",
    excerpt:
      "Explore the fascinating journey of AI from early concepts to modern machine learning.",
    views: 45200,
    gradient: "from-blue-400 to-cyan-500",
  },
  {
    title: "Climate Change: Causes and Solutions",
    category: "Science",
    excerpt:
      "A comprehensive guide to understanding climate change and sustainable practices.",
    views: 38900,
    gradient: "from-green-400 to-emerald-500",
  },
  {
    title: "Quantum Computing Explained",
    category: "Technology",
    excerpt:
      "Demystifying quantum mechanics and its applications in computing technology.",
    views: 52100,
    gradient: "from-purple-400 to-pink-500",
  },
  {
    title: "The Renaissance: A Cultural Rebirth",
    category: "History",
    excerpt:
      "Discover the pivotal era that transformed European art, philosophy, and science.",
    views: 33400,
    gradient: "from-orange-400 to-rose-500",
  },
  {
    title: "Blockchain and Cryptocurrency",
    category: "Technology",
    excerpt:
      "Understanding distributed ledger technology and its impact on modern finance.",
    views: 41800,
    gradient: "from-indigo-400 to-blue-500",
  },
  {
    title: "The Human Brain: Neuroscience 101",
    category: "Science",
    excerpt:
      "Exploring the mysteries of the most complex organ in the human body.",
    views: 48500,
    gradient: "from-red-400 to-pink-500",
  },
  {
    title: "Python Programming Guide",
    category: "Programming",
    excerpt:
      "Learn Python from basics to advanced concepts with practical examples.",
    views: 62100,
    gradient: "from-yellow-400 to-orange-500",
  },
  {
    title: "Islamic Golden Age",
    category: "History",
    excerpt:
      "The period of scientific and cultural flourishing in the Muslim world.",
    views: 29800,
    gradient: "from-amber-400 to-orange-500",
  },
  {
    title: "Web Development Best Practices",
    category: "Programming",
    excerpt:
      "Modern approaches to building fast, secure, and scalable web applications.",
    views: 54300,
    gradient: "from-teal-400 to-cyan-500",
  },
  {
    title: "Black Hole Physics",
    category: "Science",
    excerpt:
      "Understanding the universe's most mysterious and extreme objects.",
    views: 39600,
    gradient: "from-slate-500 to-gray-600",
  },
  {
    title: "The Byzantine Empire",
    category: "History",
    excerpt:
      "One of history's longest-lasting empires and its lasting influence.",
    views: 26700,
    gradient: "from-violet-400 to-purple-500",
  },
  {
    title: "Machine Learning Fundamentals",
    category: "Programming",
    excerpt: "Core concepts and algorithms that power modern AI applications.",
    views: 58900,
    gradient: "from-lime-400 to-green-500",
  },
];
